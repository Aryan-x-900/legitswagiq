...
class Bot(discord.Client):
    ...
    async def update_embeds(self):
      #  global wrong
       ...
       #global wrong             
        ...
        self.embed.set_footer(text='Aryan',icon_url='' )
        self.embed.timestamp = (datetime.datetime.utcnow())
    ...
    async def on_ready(self):
        ...
        await log.send("> **Trivia X Bot Is Updated ** ✅")
        await self.clear_results()
        await self.update_embeds()
        #await self.change_presence(activity=discord.Game(name='with '+str(len(set(self.get_all_members())))+' users'))
        await self.change_presence(activity=discord.Activity(type=1,name="With !👑⛧ƝƖƬƖƝ"))
...
def bot_with_cyclic_update_process(update_event, answer_scores):
    ...
    loop.create_task(bot.start('ODQyNDk5NDU0MjAzNDYxNjQy.YJ2Mwg.CL2-JSlZYVioHxafavPz9s2UAME'))
    loop.run_forever()
...
def selfbot_process(update_event, answer_scores):
    ...
    loop = asyncio.get_event_loop()
    loop.create_task(selfbot.start('ODM5MjYzMTMwMTAyMDA1Nzc5.YJHGsg.297oGHLyXv7ZSTwIEip5wMf-UlM',
                                   bot=True))
    loop.run_forever()
